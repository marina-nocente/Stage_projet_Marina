...
for fastqFile1 in /store/EQUIPES/REMOD/200120_novogene/Marina/RNA-seq/fastq_sans_rRNA/cleaned_filtered_C57_*paired1.fastq ; do
  ...
  then
    ...
    qsub /store/EQUIPES/REMOD/200120_novogene/Marina/bowtie2_conda_marina.qsub -o ${outputDir} -e ${outputDir} -v "fastqFile1='${fastqFile1}'","fastqFile2='${fastqFile2}'","outputDir='${outputDir}'"
	  echo "______________";

  elif [ ${cluster} == "Slurm" ]
  then
    sbatch /store/EQUIPES/REMOD/200120_novogene/Marina/bowtie2_conda_marina.qsub -o ${outputDir} -e ${outputDir} --export "fastqFile1='${fastqFile1}'","fastqFile2='${fastqFile2}'","outputDir='${outputDir}'"
	  echo "______________";
